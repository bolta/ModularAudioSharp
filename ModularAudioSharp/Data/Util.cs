...
namespace ModularAudioSharp.Data {
	static class Util {
		...
		/// <returns></returns>
		internal static U OnChannelCount<T, U>(T _, U whenMono, U whenStereo) {
		...
		internal static U OnChannelCountLazy<T, U>(T _, Func<U> whenMono, Func<U> whenStereo)
				=> OnChannelCount(_, whenMono, whenStereo)();


		internal static IEnumerable<TResult> Zip<T1, T2, T3, TResult>(this IEnumerable<T1> t1s,
