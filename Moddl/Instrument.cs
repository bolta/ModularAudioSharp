...
namespace Moddl {
	...
	class Instrument {
		...
		public Instrument(IEnumerable<ProxyController<float>> input, Node<float> output,
		...
		public Instrument(ProxyController<float> input, Node<float> output, IDictionary<string, VarController<float>> parameters,
				IEnumerable<INotable> noteUsers) : this(new [] { input }, output, parameters, noteUsers) { }

		/// <summary>
		/// this の出力を after の入力として 2 つの instrument を接続し、
		/// 全体で 1 つの instrument とする
		/// </summary>
		/// <param name="after"></param>
		/// <returns></returns>
		public Instrument Then(Instrument after) {
			// TODO input が 1 個ではない場合は例外を投げる
			after.Input[0].Source = this.Output;
			return new Instrument(this.Input, after.Output,
